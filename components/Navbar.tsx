...
export default function NavigationBar({ children }: { children: any }) {
  ...
  return (
    <>
      ...
      <Box p={10} bgColor="whitesmoke" minH="100vh">
        {children}
      </Box>
      <VStack
        bg={useColorModeValue("white", "gray.800")}
      ...
      {NAV_ITEMS.map((navItem) => (
        <Box key={navItem.label}>
          <Popover trigger={"hover"} placement={"bottom-start"}>
            <PopoverTrigger>
              <Link isExternal href={navItem.href ?? "#"}>
                <Box
                  p={2}
                  fontSize={"sm"}
                  fontWeight={500}
